...
idata = np.genfromtxt('IMU.txt', skip_header=1,
	skip_footer=1, names=['time', 'time2', 'gyrx', 'gyry', 'gyrz', 'accx', 'accy', 'accz'], autostrip=True, filling_values=0)

fig = plt.figure()

ax1 = fig.add_subplot(311)

ax1.set_title("Attitude estimate (red=onboard)")    
...
ax3.set_ylabel('yaw, zgyro(k), mag(g)')
...
maghdg = np.multiply(np.arctan2(mdata['magx'], mdata['magy']), 180 / math.pi)

ax1.plot(data['time'], data['roll'], color='r', label='roll estimate')
ax2.plot(data['time'], data['pitch'], color='r', label='pitch estimate')
ax3.plot(data['time'], data['yaw'], color='r', label='yaw estimate')

ax1.plot(data['time'], data['roll_onb'], color='b', label='roll onboard')
ax2.plot(data['time'], data['pitch_onb'], color='b', label='pitch onboard')
ax3.plot(data['time'], data['yaw_onb'], color='b', label='yaw onboard')
# ax3.plot(gdata['time'], np.math.atan2(gdata['vele'], gdata['veln']))
ax3.plot(mdata['time']/1000, maghdg, color='g', label='mag heading')
ax3.plot(idata['time']/1000, 50*idata['gyrz'], color='k', label='z gyro')
